...
os.system('prefetch SRR8185310') # get .sra file
if os.path.isdir(current_dir+'/SRR8185310'):
    print('INFO: SRA Illumina reads successfully downloaded.')
    os.system('fasterq-dump SRR8185310') # making a .fastq out of the .sra
    ...
    exit()
os.system('mv '+current_dir+'/SRR8185310 '+results_dir) # moving files to the results dir
...
spades_command = 'python3 '+current_dir+'/SPAdes-3.15.4-Linux/bin/spades.py -t 2 -k 33,55,77,99,127 --only-assembler -s '+results_dir+'/SRR8185310.fastq -o '+results_dir+'/SRR8185310_assembly/' # SPAdes command line
log_output.write(spades_command+'\n')
os.system(spades_command) # run SPAdes
...
records = list(SeqIO.parse(results_dir+'/SRR8185310_assembly/scaffolds.fasta', 'fasta')) # parsing .fasta
final_contigs = 0
assembly_size = 0
with open(results_dir+'/SRR8185310_assembly_1000bp.fasta', 'w') as outfile:
    for item in records:
        if len(str(item.seq)) > 1000: # check for contigs with >1000 bp in length
...
gms2_command = 'perl '+current_dir+'/gms2_linux_64/gms2.pl --seq '+results_dir+'/SRR8185310_assembly_1000bp.fasta --genome-type bacteria --output '+results_dir+'/SRR8185310_anno.gtf --format gtf --faa '+results_dir+'/SRR8185310_anno_AA.fasta' # gms2 command line
os.system(gms2_command) # run gnms2
...
if os.path.isfile(current_dir+'/Ecoli_db.phr') == False and os.path.isfile(current_dir+'/Ecoli_db.pin') == False and os.path.isfile(current_dir+'/Ecoli_db.psq') == False: # checks if db exists
    print('INFO: BLAST database not found. Attempting to make one.')
    os.system('makeblastdb -in Ecoli.fasta -out Ecoli_db -title Ecoli_db -dbtype prot') # run makeblastdb
...
blast_command = NcbiblastpCommandline(query = results_dir+'/SRR8185310_anno_AA.fasta', db = 'Ecoli_db', num_threads = 2, max_target_seqs = 1, outfmt = '10 qseqid sseqid pident qcovs', out = results_dir+'/SRR8185310_blastp_results.csv') # blastp command line
stdout, stderr = blast_command() # run blastp
if os.path.isfile(results_dir+'/SRR8185310_blastp_results.csv'):
    ...
    exit()
os.system('mv '+current_dir+'/Ecoli_db.phr '+results_dir) # moving output files to results dir
os.system('mv '+current_dir+'/Ecoli_db.pin '+results_dir)
os.system('mv '+current_dir+'/Ecoli_db.psq '+results_dir)

with open(results_dir+'/SRR8185310_blastp_results.csv', 'r') as blast_file: # opens blastp output file to assess how many CDS have hits
    ...
    if cds_n > 4140:
        ...
        print('GeneMarkS-2 found the same number of CDS as the RefSeq.')
...
os.system('prefetch SRR1411276') # gets .sra file
if os.path.isdir(current_dir+'/SRR1411276'):
    print('INFO: SRA Illumina reads successfully downloaded.')
    os.system('fasterq-dump SRR1411276') # makes a .fastq out of the .sra
    ...
    print('INFO: Could not download SRA Illumina reads. Check if SRA toolkit has been correctly installed or if the given accession number is correct, and try again.')
    exit()
os.system('mv '+current_dir+'/SRR1411276 '+results_dir) # moving files to the results dir
...
os.system('bowtie-build '+results_dir+'/SRR8185310_assembly_1000bp.fasta NC_000913') # run bowtie
if os.path.isfile(current_dir+'/NC_000913.1.ebwt') and os.path.isfile(current_dir+'/NC_000913.2.ebwt') and os.path.isfile(current_dir+'/NC_000913.3.ebwt') and os.path.isfile(current_dir+'/NC_000913.4.ebwt') and os.path.isfile(current_dir+'/NC_000913.rev.1.ebwt') and os.path.isfile(current_dir+'/NC_000913.rev.2.ebwt'):
    ...
    print('INFO: Could not index the E. coli K-12 assembled genome. Check if input file exists or Bowtie is correctly installed.')
    exit()
...
os.system('tophat2 --no-novel-juncs -o '+results_dir+'/SRR1411276_mapped_NC_000913 NC_000913 '+results_dir+'/SRR1411276.fastq') # tophat2 command line
if os.path.isfile(results_dir+'/SRR1411276_mapped_NC_000913/accepted_hits.bam'):
    ...
    exit()
os.system('mv '+current_dir+'/NC_000913.fa '+results_dir) # moving output files to the results dir
...
os.system('cufflinks -o '+results_dir+'/SRR1411276_cufflinks -p 2 -G '+results_dir+'/SRR8185310_anno.gtf '+results_dir+'/SRR1411276_mapped_NC_000913/accepted_hits.bam') # cufflinks command line
...
print('INFO: Summarizing Cufflinks output.') # gets seqname, start, end, strand, and fpkm values of mapped reads
